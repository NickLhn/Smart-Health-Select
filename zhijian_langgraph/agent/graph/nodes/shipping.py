...
from agent.graph.formatters import ShippingFormatter
...
def shipping_flow_node(state: "GraphState") -> dict[str, Any]:
    """物流查询流程."""
    from agent.settings import load_settings
    ...
    # 有订单号 -> 查详情
    if order_no:
        return _query_shipping_detail(tools, user_token, order_no, request_id, formatter)
    
    # 无订单号 -> 查列表
    return _query_shipping_list(tools, user_token, request_id, formatter)


def _query_shipping_detail(
    tools: "ToolsClient",
    user_token: str,
    order_no: str,
    request_id: str | None,
    ...
    err = _error_reply(payload, "未找到该订单或配送信息。")
    if err:
        return err
    ...
    reply = formatter.format_detail(delivery)
    return {"reply": reply}


def _query_shipping_list(
    tools: "ToolsClient",
    user_token: str,
    request_id: str | None,
    formatter: ShippingFormatter,
    ...
    err = _error_reply(payload, "暂时没有查到可跟踪的配送订单。")
    if err:
        return err
